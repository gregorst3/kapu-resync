...
function SyncPeers()
{
        ...
        PEERS=$(curl -s "https://walletapi.kapu.one/api/peers" | jq '[.peers[]]' | jq 'sort_by(-.height)' | jq '.[] |= del(.status,.os,.version,.errors,.broadhash,.clock,.delay,.nonce,.height) ' | jq '[limit(5;.[])]')
